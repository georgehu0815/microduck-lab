...
def is_studio_policy_path(path: str | Path) -> bool:
    ...
    return (
        ...
        and relative.parts[0] in TRUSTED_RECIPES
...
def _validate_onnx_contract(model: Any, policy_path: Path) -> None:
    initializer_names = {initializer.name for initializer in model.graph.initializer}
    inputs = [
        value for value in model.graph.input if value.name not in initializer_names
    ]
    outputs = list(model.graph.output)
    if policy_path.parent.parent.name == "basketball":
...
def _load_onnx_metadata(policy_path: Path) -> dict[str, Any] | None:
    ...
    _validate_onnx_contract(model, policy_path)
