...
以下七个核心模块按本版快照完整收录；注释、导入与实现保持原样。代码源文件与 SHA-256 清单随书提供。其他适配器、完整 Studio CLI、篮球评估器、共享模型、viewer 接口和测试位于 `source/` 与 `supporting-source.zip`。快照不是独立软件包；运行需要完整工作区、机器人资产和所列依赖。复制代码请使用 Markdown 或 `.py` 文件，PDF 中的视觉换行不是额外语法。
